console.log("--- SCRIPT VERSION 3.0 --- RUNNING ---");
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // --- Constants ---
    // This token is now hardcoded for direct GitHub API access.
    ...
    const saveTasksToGithub = async () => {
        try {
            if (!GITHUB_TOKEN) {
                console.error("CRITICAL: GitHub token is missing from script.js");
                showNotification("Error: GitHub token is not set in the code.", true);
                return;
            }
            ...
            try {
                ...
                if (response.ok) {
                    ...
                    throw new Error(`Could not fetch file SHA. Status: ${response.status}`);
                }
            } catch (error) {
        ...
        try {
            ...
            if (!response.ok) {
                if (response.status === 404) {
                    ...
                    settings = localData.settings || { suggestionStrategy: 'hardFirst' };
                    showNotification("Created new task list. First save will commit to GitHub.", false);
                } else {
